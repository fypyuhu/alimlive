...
class browser extends uploader {
    ...
    public function __construct() {
        ...
        if (Input::has('dir')&& Request::isMethod('post')) {
            $dir = $this->checkInputDir(Input::get('dir'), true, false);
            if ($dir === false) unset(Input::get('dir'));
            Input::replace(['dir' => $dir]);
        }

        if (Input::has('dir') && Request::isMethod('get')) {
            $dir = $this->checkInputDir(Input::get('dir'), true, false);
            if ($dir === false) unset(Input::get('dir'));
            Input::replace(['dir' => $dir]);
        }
    ...
    protected function act_browser() {
        if (isset(Input::get('dir'))) {
            $dir = "{$this->typeDir}/{Input::get('dir')}";
            if ($this->checkFilePath($dir) && is_dir($dir) && is_readable($dir))
                $this->session['dir'] = path::normalize("{$this->type}/{Input::get('dir')}");
        }
        return $this->output();
    }
    ...
    protected function act_thumb() {
        ...
        $dir = $this->getDir();
        $file = "{$this->thumbsTypeDir}/{Input::get('dir')}/${_GET['file']}";
    ...
    protected function act_chDir() {
        ...
        $this->session['dir'] = "{$this->type}/{Input::get('dir')}";
        $dirWritable = dir::isWritable("{$this->config['uploadDir']}/{$this->session['dir']}");
    ...
    protected function act_renameDir() {
        if (!$this->config['access']['dirs']['rename'] ||
            !Input::has('dir') ||
            !strlen(rtrim(rtrim(trim(Input::get('dir')), "/"), "\\")) ||
            !isset($_POST['newName']) ||
        ...
        $thumbDir = "$this->thumbsTypeDir/{Input::get('dir')}";
        if (is_dir($thumbDir))
    ...
    protected function act_delete() {
        ...
        $thumb = "{$this->thumbsTypeDir}/{Input::get('dir')}/{$_POST['file']}";
        ...
        foreach($_POST['files'] as $file) {
            ...
            elseif (!@copy($path, "$dir/$base"))
                ...
                if (is_file($fromThumb) && is_readable($fromThumb)) {
                    $toThumb = "{$this->thumbsTypeDir}/{Input::get('dir')}";
                    if (!is_dir($toThumb))
    ...
    protected function act_downloadDir() {
        $dir = $this->postDir();
        if (!Input::has('dir') || $this->config['denyZipDownload'])
            $this->errorMsg("Unknown error.");
